...
if __name__ == "__main__":
    ...
    jetstream_httpd = MozHttpd(
        port=8001,
        docroot=os.path.join(
            build.topsrcdir, "third_party", "webkit", "PerformanceTests", "JetStream"
        ),
        path_mappings=path_mappings,
    )
    jetstream_httpd.start(block=False)
    print("started JetStream server on port 8001")

    motionmark_httpd = MozHttpd(
        port=8002,
        docroot=os.path.join(
            build.topsrcdir, "third_party", "webkit", "PerformanceTests", "MotionMark"
        ),
        path_mappings=path_mappings,
    )
    motionmark_httpd.start(block=False)
    ...
    with TemporaryDirectory() as profilePath:
        ...
        jetstream_interpolation = {"server": "%s:%d" % jetstream_httpd.httpd.server_address}
        motionmark_interpolation = {"server": "%s:%d" % motionmark_httpd.httpd.server_address}
        ...
        if ret:
            ...
            if logfile:
                ...
                with open(logfile) as f:
                    print(f.read())
            motionmark_httpd.stop()
            jetstream_httpd.stop()
            sp3_httpd.stop()
            httpd.stop()
